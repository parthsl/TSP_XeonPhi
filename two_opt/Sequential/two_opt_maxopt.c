 ...
 * If distance reduces than the previous one then note the elements range to be reversed and cost change that can be obtained. 
 * After traversing through ehole search space reverse the elements between min_cirtcuit[i+1] to min_circuit[j] where i and j is the nodes with maximal cost change.
