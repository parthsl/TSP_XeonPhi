...
/*
 * @Author : Sourabh Jain
 * Parallel OpenMP implementation of 2opt method.
 * Each thread forks private copy of minimum tour and results are combined back to get best tour.
 */
double** G;
...
void two_opt(double** G, int* min_circuit, int cities){
	for(int i=0;i<cities-1;i++){
		...
		for(int j=i+2;j<cities-1+z;j++){
			...
			if(f_dist>s_dist){
				//printf("benefit should be %lf : ",f_dist-s_dist);
				//printf("before\n");
				//print_tour(G,min_circuit,cities);
				rev_arr(min_circuit,i+1,j);
				//printf(" %lf\n reversed = %d %d %d %d\n",find_tour_length(G,min_circuit,cities),i_city,i_next_city,j_city,j_next_city);
				//print_tour(G,min_circuit,cities);
			}
		}
	}
