...
int main(int argc, char** argv) // argv1 = filename argv2 = threads
{
	...
	if(num_local != master){
		...
		for(nd i=0; i<2; i++)
			G[i] = (double*)malloc(sizeof(double)*total_cities);
	}

	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Bcast(&G[0][0],total_cities, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	MPI_Bcast(&G[1][0],total_cities, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
...
#pragma omp parallel 
	...
	char mach_name[100];int mach_len;
