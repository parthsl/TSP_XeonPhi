...
void two_opt(double** G, int* min_circuit, int cities){
	for(int i=0;i<cities-1;i++){
		...
		for(int j=i+2;j<cities-1;j++){
			...
			if(f_dist>s_dist){
				min_circuit[j] = min_circuit[i+1] + min_circuit[j] - (min_circuit[i+1] = min_circuit[j]);
				rev_arr(min_circuit,i+2,j-1);
			}
		}
	}
}
